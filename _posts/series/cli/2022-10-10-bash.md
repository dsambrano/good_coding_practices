...
When you first learn bash there are three must learn commands. These are really important commands because they allow you to navigate around the your file system:

- `pwd`: Print Working Directory
- `cd`: Change Directory
- `ls`: LiSt (Contents)

An analogy for these commands is imaging you are on a random street in a US city that you don't know. 
...
The `pwd` command shows you where you are currently located and is equivalent to `getwd()` in `R` or `os.getcwd()` in `python`. 
