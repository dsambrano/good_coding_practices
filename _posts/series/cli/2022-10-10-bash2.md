...
So my goal is to motivate you by showing you fun useful ideas/commands to spark your creativity to inspire you to create something.
...
Again, like the rest of this post, this is just a taste of how to edit file, and with these skills you will be able to make some changes, but will not be super efficient.
...
First, to get some more practice make a new folder in your Documents folder (or you Desktop if you prefer to see it) and inside that use one of the two methods above to make a blank file.
...
So in this case, to save, you just hit `^` + `O`, where the `^` means the Control key on your keyboard, usually the fall left corner. And the `O` is the letter not the number (you don't need the shift key even though it displays in as a capital letter).
...
Finnaly, you can exit `nano` as shown in the bottom that is with `^` + `X`, which again is the Control key and the letter. 
...
You can move around (`cd`), look around (`ls`), make/change things (`mkdir`, `touch`, `echo`, `nano`).
...
By this point, you probably have a good intuitive sense of the structure of commands, so feel free to skip to the [list]
...
- `alias` to make long command shorted, but making your own alternative command name
...
- `which` shows you were a command lives on your system
